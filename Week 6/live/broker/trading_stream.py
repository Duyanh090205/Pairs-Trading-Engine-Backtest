...
class TradingStreamHandler:
    ...
    def _update_order_row(self, conn, coid, broker_id, status, filled_qty, fill_price, ev_type) -> None:
        ...
        if existing is not None and existing["status"].lower() in self._TERMINAL_STATUSES \
                and status.lower() not in self._TERMINAL_STATUSES:
            ...
            return
        filled_ts = ts if status.lower() == "filled" else None
        conn.execute(
    ...
    def _maybe_open_position(self, conn, coid: str) -> None:
        ...
        if not all(l["status"].lower() == "filled" for l in legs):
            return
